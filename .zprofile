...
if { uname | grep -q Linux; } && [ -e $HOME/liuxiang ] ; then
  ...
  _new_path=($HOME/ServerApps/bin $HOME/bin $HOME/Library/Python/3.9/bin $HOME/Library/Python/2.7/bin)
    for i in "${path[@]}"
    do
      if [ $HOME/ServerApps/bin != "$i" -a $HOME/bin != "$i" -a $HOME/Library/Python/2.7/bin != "$i" -a $HOME/Library/Python/3.9/bin != "$i" ]; then
            _new_path+=$i
        fi
    done
fi
