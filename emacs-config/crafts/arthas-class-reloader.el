...
(defun arthas-class-reload--reload-class (option)
  (let* ((context (plist-get option :context))
         (name (plist-get option :name))
         (cmd (format "%s/arthas-class-reload-class.sh '%s' '%s' '%s'" arthas-class-reload-dir context name arthas-class-reload--class-file)))
    (message (shell-command-to-string cmd))))

(defclass arthas-class-reload--docker-source (helm-source-sync)
  ((candidates :initform 'arthas-class-reload--docker-options)
   (action :initform 'arthas-class-reload--reload-class)))

(setq arthas-class-reload--docker-options-list
      (helm-make-source "Containers for Arthas Class Reloader" 'arthas-class-reload--docker-source))

(defun arthas-class-reload--read-container ()
  (let ((default-directory "~"))
    (helm-other-buffer '(arthas-class-reload--docker-options-list) "*helm-ascreloader*")))

(defun arthas-class-reload (&optional source-file)
  (interactive)
    ...
    (if class-file
        (if (arthas-class-reload--check-class-file-time source-file class-file)
            (let* ((arthas-class-reload--class-file class-file))
                   (arthas-class-reload--read-container)))
