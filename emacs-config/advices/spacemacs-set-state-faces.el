...
(defun lx/optimize-theme-colors-and-font (&rest args)
  ...
  (when (string= "catppuccin" spacemacs--cur-theme)
    ...
    ;; Ediff
    (face-spec-set 'ediff-current-diff-A '((t (:background "#4d2f3a" :foreground "#ed8796" :extend t))))
    (face-spec-set 'ediff-current-diff-B '((t (:background "#2d5040" :foreground "#a6da95" :extend t))))
    (face-spec-set 'ediff-current-diff-C '((t (:background "#2a3548" :foreground "#8aadf4" :extend t))))
    (face-spec-set 'ediff-current-diff-Ancestor '((t (:background "#253c38" :foreground "#8bd5ca" :extend t))))
    (face-spec-set 'ediff-fine-diff-A '((t (:background "#ed8796" :foreground "#24273a" :inherit bold :extend t))))
    (face-spec-set 'ediff-fine-diff-B '((t (:background "#a6da95" :foreground "#24273a" :inherit bold :extend t))))
    (face-spec-set 'ediff-fine-diff-C '((t (:background "#8aadf4" :foreground "#24273a" :inherit bold :extend t))))
    (face-spec-set 'ediff-fine-diff-Ancestor '((t (:background unspecified :inherit bold :extend t))))
    (face-spec-set 'ediff-even-diff-A '((t (:background "#1e2030" :extend t))))
    (face-spec-set 'ediff-even-diff-B '((t (:background "#1e2030" :extend t))))
    (face-spec-set 'ediff-even-diff-C '((t (:background "#1e2030" :extend t))))
    (face-spec-set 'ediff-even-diff-Ancestor '((t (:background "#1e2030" :extend t))))
    (face-spec-set 'ediff-odd-diff-A '((t (:background "#181926" :extend t))))
    (face-spec-set 'ediff-odd-diff-B '((t (:background "#181926" :extend t))))
    (face-spec-set 'ediff-odd-diff-C '((t (:background "#181926" :extend t))))
    (face-spec-set 'ediff-odd-diff-Ancestor '((t (:background "#181926" :extend t))))
    ;; System
    (add-to-list 'default-frame-alist '(ns-appearance . dark))
    (shell-command-to-string "defaults write org.gnu.Emacs TransparentTitleBar DARK")
    (shell-command-to-string "tmux set-option -g status-style bg=colour235,fg=colour244"))
