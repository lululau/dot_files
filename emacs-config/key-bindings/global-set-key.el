...
(global-set-key (kbd "s-;") #'(lambda () (interactive) (lx/run-in-pry-vterm (cdr (assoc "pry" inf-ruby-implementations)) "*pry*")))
...
(global-set-key (kbd "s-r 0") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh lx.kt" "*vterm-ssh-lx.kt*" nil t))))
(global-set-key (kbd "s-r 1") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh lcl.devb" "*vterm-ssh-lcl.devb*" nil t))))
(global-set-key (kbd "s-r 2") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh ceres.devb2" "*vterm-ssh-ceres.devb2*" nil t))))
(global-set-key (kbd "s-r 3") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh lcl.pro1" "*vterm-ssh-lcl.pro1*" nil t))))
(global-set-key (kbd "s-r 4") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh lcl.pro2" "*vterm-ssh-lcl.pro2*" nil t))))
(global-set-key (kbd "s-r 5") #'(lambda () (interactive) (let ((process-environment '("SSH_INTERACTIVE=1"))) (lx/run-shell-in-vterm "ssh lcl.pro3" "*vterm-ssh-lcl.pro3*" nil t))))
...
(global-set-key (kbd "s-r l 5") #'(lambda () (interactive) (lx/run-in-vterm "~/bin/vrl lcl-devb-admin -a" "*vterm-vrl-lcl-admin*" nil t)))
(global-set-key (kbd "s-r p 0") #'(lambda () (interactive) (lx/run-in-vterm "~/.rvm/gems/ruby-3.1.0/bin/prize -h 127.0.0.1 -d 0" "*vterm-prize-localhost*" nil t)))
(global-set-key (kbd "s-r p 1") #'(lambda () (interactive) (lx/run-in-vterm "~/.rvm/gems/ruby-3.1.0/bin/prize -h 39.97.213.239 -d 0" "*vterm-prize-lcl-devb*" nil t)))
(global-set-key (kbd "s-r p 2") #'(lambda () (interactive) (lx/run-in-vterm "~/.rvm/gems/ruby-3.1.0/bin/prize -h 39.97.213.239 -d 1" "*vterm-prize-md-dev*" nil t)))
...
(global-set-key (kbd "s-r s-c bU") #'(lambda () (interactive) (let ((vterm-kill-buffer-on-exit nil)) (lx/run-in-vterm "brew upgrade ; brew post-update ; brew cleanup ; brew list --casks | xargs -I {} brew upgrade --cask {}" "*vterm-cmd-brew-upgrade-cleanup*" nil t))))
(global-set-key (kbd "s-r s-c lp") #'(lambda () (interactive) (let ((vterm-kill-buffer-on-exit nil)) (lx/run-in-vterm "sudo lsof -Pn -iTCP -sTCP:LISTEN" "*vterm-cmd-listening-ports*" nil t))))
(global-set-key (kbd "s-r s-c gs") #'(lambda () (interactive) (let* ((vterm-kill-buffer-on-exit nil)
...
(global-set-key (kbd "s-r s-c gb") #'(lambda () (interactive) (let* ((vterm-kill-buffer-on-exit nil)
                                                                     (root (projectile-project-root))
                                                                     (root-base-name (car (last (split-string root "/" t))))
                                                                     (buffer-name (format "*vterm-cmd-git-multi-branch-%s*" root-base-name)))
                                                                (lx/run-in-vterm "git multi-branch" buffer-name root t))))
(global-set-key (kbd "s-r s-c gl") #'(lambda () (interactive) (let* ((vterm-kill-buffer-on-exit nil)
                                                                    (root (projectile-project-root))
                                                                    (root-base-name (car (last (split-string root "/" t))))
