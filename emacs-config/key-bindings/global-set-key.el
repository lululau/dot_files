...
(global-set-key (kbd "s-r q") #'(lambda () (interactive)
                                  (if (bound-and-true-p arql-env)
                                      (lx/run-in-vterm (format "~/.rvm/gems/ruby-2.7.0/bin/arql -e %s" arql-env) (format "*arql-%s*" arql-env) (projectile-project-root))
                                    (lx/run-in-vterm "~/.rvm/gems/ruby-2.7.0/bin/arql -e lcldevb" "*arql-lcldevb*" (projectile-project-root) t))))
